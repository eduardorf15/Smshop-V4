...
export async function debugMercadoLivreImport(input) {
  ...
  return {
    ...
    tokenValid: Boolean(trace.auth?.tokenValid),
    tokenExpired: Boolean(trace.auth?.tokenExpired),
    authMode: trace.auth?.authMode || "fallback",
    auth: trace.auth,
    endpointResponses: apiAttempts.map((attempt) => ({
